...
Global SessionLocalSync — no per-task engine creation.
None-check (not truthiness) for zero-valued FAIR params.
"""
        ...
        for risk in open_risks:
            ...
            try:
                ...
                if old_ale and abs(calc["ale_mean_gbp"] - old_ale) > 50_000:
                    logger.info("ALE delta > £50K", risk_ref=risk.risk_ref,
                                old=round(old_ale), new=round(calc["ale_mean_gbp"]))
                updated += 1
